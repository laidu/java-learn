#! /bin/bash
# es docker start script.

cluster_name=$1
es_root=/Users/laidu/Files/data/docker/elasticsearch
pubilc_path=/Users/laidu/Files/data/docker/upload

#docker run  -d --restart=always \
docker run --rm -d --name es-dev \
	-p 9200:9200 \
	-p 9300:9300 \
	-e cluster.name=${cluster_name} \
	-e discovery.type=single-node \
	-e ELASTIC_PASSWORD=elastic \
	-e ES_JAVA_OPTS="-Xms3g -Xmx3g" \
	-v ${es_root}/config/elasticsearch.yml:/usr/share/elasticsearch/config/elasticsearch.yml \
	-v ${es_root}/data:/usr/share/elasticsearch/data \
	-v ${pubilc_path}/x-pack-core-6.2.2.jar:/usr/share/elasticsearch/plugins/x-pack/x-pack-core/x-pack-core-6.2.2.jar \
	-v ${pubilc_path}:/root \
	-v /etc/hosts:/etc/hosts \
	daocloud.io/laidu/elasticsearch_6

# {"license":{"uid":"c6570128-85c2-4f72-8d8f-b1425455b9ee","type":"platinum","issue_date_in_millis":1515369600000,"expiry_date_in_millis":2524579200999,"max_nodes":10000,"issued_to":"elastic","issuer":"elastic","signature":"","start_date_in_millis":1515369600000}}
